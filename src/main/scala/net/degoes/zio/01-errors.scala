...
object ErrorRecoveryOrElse extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(ZIO.succeed("everything is okay")).exitCode
}
...
object ErrorShortCircuit extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(printLine("saved")).exitCode
}
...
object ErrorRecoveryCatchAll extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.catchAll(errors =>  printLine(errors)).exitCode
}
...
object ErrorRecoveryFoldZIO extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.foldZIO(e => printLine(e), a => printLine(a)).exitCode
}
...
object AlarmApp extends App {
  ...
  lazy val getAlarmDuration: ZIO[Has[Console], IOException, Duration] = {
    def parseDuration(input: String): IO[NumberFormatException, Duration] =
      IO.attempt{
        Duration.fromMillis(input.toInt * 1000)
      }.refineToOrDie[NumberFormatException]

    def fallback(input: String): ZIO[Has[Console], IOException, Duration] =
      ZIO.fail(new IOException(s"Unexpedted input: $input")) *> getAlarmDuration
...
object Sandbox extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    composed.sandbox.cause.flatMap(cause => printLine(cause)).exitCode
}
